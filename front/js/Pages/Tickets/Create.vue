  ...
  <AppLayout>
    <template #header>
      <div class="flex items-center gap-3">
        <Link :href="route('tickets.index')" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">← Tickets</Link>
        <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Novo Ticket</h2>
      </div>
    </template>
      ...
      <form @submit.prevent="submit" class="space-y-4" enctype="multipart/form-data">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Anexo (opcional — JSON ou TXT, máx. 2MB)
          </label>
          <input
            type="file"
            accept=".json,.txt"
            class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:bg-indigo-50 file:text-indigo-700 dark:file:bg-indigo-900 dark:file:text-indigo-300"
            @change="e => form.attachment = e.target.files[0]"
          />
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Se um arquivo for anexado, será processado em fila automaticamente.</p>
        ...
        <div class="flex gap-3 pt-2">
          ...
          <Link :href="route('tickets.index')" class="px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">Cancelar</Link>
        </div>
      </form>
    </div>
  </AppLayout>
